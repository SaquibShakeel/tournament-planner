...
import { Match, matchGenerator } from "../components/matchGenerator";
import MatchCard from "../components/MatchCard";

const Tournament = () => {
  const participants = useSelector((state: any) => state.participants);
  const temp = [];
  temp.push(matchGenerator(participants));
  ...
  const nextRoundHandler = () => {
    try {
      ...
      const nextRound: Match[] = matchGenerator(qualifiedTeams);
      setMatches((prev)=> [...prev, nextRound]);
    } catch (e) {
      console.log(e);
    }
  };
